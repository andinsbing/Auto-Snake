...
public:
    ...
    void  setGameItem(const Position & pos, GameItem gameItem)
	{
    ...
    void  setGameItem(const std::vector<Position>& poses, Global::GameItem gameItem)
	{
		for (auto& i : poses)
		{
    ...
    GameItem  getGameItem(const Position & pos) const
	{
		ASSERT_POS_LEGAL(pos.x, pos.y);
    ...
    std::vector<Position>  getSet(const GameItem gameItem) const
	{
    ...
    Position const  firsOf(GameItem gameItem) const
	{
		for (int i = 0; i < HEIGHT; i++)
		{
			for (int j = 0; j < WIDTH; j++)
